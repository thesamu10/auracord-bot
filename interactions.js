...
async function handleButton(client, interaction) {
  ...
  if (id.startsWith('reg_join_')) {
    ...
    if (already) return interaction.reply({ content: '❌ Sei già registrato.', ephemeral: true });

    // Registra direttamente — epic name verrà da Yunite API
    ...
    // Assegna ruolo
    const role = interaction.guild.roles.cache.get(lobbyData.roleId);
    ...
    if (lobbyData.registrations.length === 50 && !lobbyData.fillsOpen) {
      ...
      await kvSet(`lobby_${scrimId}_${lobbyNum}`, lobbyData);
      const fillsCh = interaction.guild.channels.cache.get(lobbyData.channels.fills);
      if (fillsCh) {
        await fillsCh.permissionOverwrites.edit(interaction.guild.roles.everyone, {
          ViewChannel: true, SendMessages: false,
        });
        const { EmbedBuilder } = require('discord.js');
        const embed = new EmbedBuilder()
          .setTitle('🎮 Fill Requests Aperte')
          .setDescription(`Abbiamo raggiunto **50 registrazioni**.
Se vuoi partecipare come fill, contatta lo staff.`)
          .setColor(0x7c3aed);
    ...
    return interaction.reply({
      content: `✅ Registrato con successo! Ti è stato assegnato il ruolo \`${lobbyData.roleName}\`.`,
      ephemeral: true,
    });
  }
